...
//카메라가 포인터 형식
void XSelect::SetMatrix(D3DXMATRIX* pWorld, D3DXMATRIX* pView, D3DXMATRIX* pProj, HWND hWnd, RECT rt)
{
	if (pWorld != NULL)
	{
		m_matWorld = *pWorld;
...
void XSelect::Update(HWND hWnd, RECT rt)
{
	POINT ptCursor;
	GetPhysicalCursorPos(&ptCursor);		//마우스 커서를 얻음
	ScreenToClient(hWnd, &ptCursor);		//스크린좌표의 마우스 좌표를 얻음


	D3DXVECTOR3 k;

	//화면좌표 -> 투영좌표로 변환 [책103장]
	k.x = (((2.0f*ptCursor.x) / rt.right) - 1) / m_matProj._11;	//11,22 성분이 정점에 미치기에 역행렬 곱 작업 대체함
	k.y = -(((2.0f*ptCursor.y) / rt.bottom) - 1) / m_matProj._22;
	k.z = 1.0f;
