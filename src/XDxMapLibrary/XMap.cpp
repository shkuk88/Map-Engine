...
void XMap::CreateBuffer(ID3D11Device* pDevice)
{
	m_pVertexBuffer.Attach(m_Object.CreateBuffer(pDevice, &m_VertexList.at(0), m_VertexList.size(), sizeof(PNCT_Vertex), D3D11_BIND_VERTEX_BUFFER));
	m_pIndexBuffer.Attach(m_Object.CreateBuffer(pDevice, &m_dwIndexList.at(0), m_dwIndexList.size(), sizeof(DWORD), D3D11_BIND_INDEX_BUFFER));
	m_pConstantBuffer.Attach(m_Object.CreateBuffer(pDevice, &m_ConstantDataMatrix, 1, sizeof(ConstantBuffer_Matrix), D3D11_BIND_CONSTANT_BUFFER));
	m_pLightConstantBuffer.Attach(m_Object.CreateBuffer(pDevice, &m_LightData, 1, sizeof(ConstantBuffer_Light), D3D11_BIND_CONSTANT_BUFFER));
}

void XMap::CreateShader(ID3D11Device* pDevice, TCHAR* szMapShader, TCHAR* szOnlyColorShader, char* szVSFunctionName, char* szPSFunctionName)
{
	m_pVS[Tex_Have].Attach(m_Object.CreateVertexShader(pDevice,szMapShader, szVSFunctionName, &m_pVSBuf));
	m_pPS[Tex_Have].Attach(m_Object.CreatePixelShader(pDevice,szMapShader, szPSFunctionName, &m_pPSBuf));
	m_pVS[Tex_None].Attach(m_Object.CreateVertexShader(pDevice,szOnlyColorShader, szVSFunctionName, &m_pVSBuf));
	m_pPS[Tex_None].Attach(m_Object.CreatePixelShader(pDevice,szOnlyColorShader, szPSFunctionName, &m_pPSBuf));
}
...
bool XMap::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, float fDistance,TCHAR* szTexture, TCHAR* szHeightTexture, TCHAR* szMapShader, TCHAR* szOnlyColorShader, char* szVSFunctionName, char* szPSFunctionName)
{
	...
	CreateShader(pDevice, szMapShader, szOnlyColorShader, szVSFunctionName, szPSFunctionName);
	...
	m_pVertexLayout.Attach(m_Object.CreateInputlayout(pDevice, m_pVSBuf->GetBufferSize(), m_pVSBuf->GetBufferPointer(), layout, sizeof(layout) / sizeof(layout[0])));
	if (szTexture)
	{
...
bool XMap::Frame(const float& spf, const float& accTime)noexcept
{
	return true;
}
