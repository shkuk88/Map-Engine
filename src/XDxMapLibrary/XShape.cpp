...
bool XLineShape::Init()noexcept
{
	SetVertex();
	m_pVertexBuffer.Attach(m_Object.CreateBuffer(DxManager::Get().GetDevice(), &m_VertexList_Line.at(0), m_VertexList_Line.size(), sizeof(PC_Vertex), D3D11_BIND_VERTEX_BUFFER));
	m_pConstantBuffer.Attach(m_Object.CreateBuffer(DxManager::Get().GetDevice(), &m_ConstantDataMatrix, 1, sizeof(ConstantBuffer_Matrix), D3D11_BIND_CONSTANT_BUFFER));
...
{
	Init();
	m_pVS.Attach(m_Object.CreateVertexShader(DxManager::Get().GetDevice(), szVertexShaderFile, szVSFunctionName, &m_pVSBuf));
	m_pPS.Attach(m_Object.CreatePixelShader(DxManager::Get().GetDevice(), szPixelShaderFile, szPSFunctionName, &m_pPSBuf));
	...
	m_pVertexLayout.Attach(m_Object.CreateInputlayout(DxManager::Get().GetDevice(), m_pVSBuf->GetBufferSize(), m_pVSBuf->GetBufferPointer(), layout, sizeof(layout) / sizeof(layout[0])));
	return true;
}
...
bool XLineShape::Frame(const float& spf, const float& accTime)noexcept
{
	return true;
}

bool XLineShape::Render(ID3D11DeviceContext* pContext)noexcept
{
...
bool XLineShape::Release()noexcept
{
	return true;
}
...
bool XBoxShape::Init()noexcept
{
	SetVertex();
	m_pVertexBuffer.Attach(m_Object.CreateBuffer(DxManager::Get().GetDevice(), &m_VertexList.at(0), m_VertexList.size(), sizeof(PNCT_Vertex), D3D11_BIND_VERTEX_BUFFER));
	SetIndex();
	m_pIndexBuffer.Attach(m_Object.CreateBuffer(DxManager::Get().GetDevice(), &m_dwIndexList.at(0), m_dwIndexList.size(), sizeof(DWORD), D3D11_BIND_INDEX_BUFFER));
	m_pConstantBuffer.Attach(m_Object.CreateBuffer(DxManager::Get().GetDevice(), &m_ConstantDataMatrix, 1, sizeof(ConstantBuffer_Matrix), D3D11_BIND_CONSTANT_BUFFER));
...
bool XBoxShape::Frame(const float& spf, const float& accTime)noexcept
{
	return true; 
}

bool XBoxShape::Render(ID3D11DeviceContext* pContext)noexcept
{
	pContext->VSSetShader(m_pVS.Get(), NULL, 0);
	pContext->PSSetShader(m_pPS.Get(), NULL, 0);
...
bool XBoxShape::Release()noexcept
{
	return true; 
}
...
bool XPlaneShape::Init()noexcept	
{
	SetVertex();
	m_pVertexBuffer.Attach(m_Object.CreateBuffer(DxManager::Get().GetDevice(), &m_VertexList.at(0), m_VertexList.size(), sizeof(PNCT_Vertex), D3D11_BIND_VERTEX_BUFFER));
	SetIndex();
...
bool XPlaneShape::Create(TCHAR* szTexture, TCHAR* szVertexShaderFile, TCHAR* szPixelShaderFile, char* szVSFunctionName, char* szPSFunctionName)
{
	...
	if(szTexture != NULL)	m_pTextureSRV = I_SRV.Find(I_SRV.Add(DxManager::Get().GetDevice(), szTexture));
	return true;
}

bool XPlaneShape::Frame(const float& spf, const float& accTime)noexcept
{
	return true;
}

bool XPlaneShape::Render(ID3D11DeviceContext* pContext) noexcept
{
	pContext->VSSetShader(m_pVS.Get(), NULL, 0);
	pContext->PSSetShader(m_pPS.Get(), NULL, 0);
...
bool XPlaneShape::Release() noexcept
{
	return true;
}
