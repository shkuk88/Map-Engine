...
bool XObjectController::Frame(const float& spf, const float& accTime) { return true; }
bool XObjectController::Frame(ID3D11DeviceContext* pContext, const float& spf, const float& accTime)
{
	...
	for (auto pObject : I_Object.m_ObjectList)
	{
		...
		{
			pObject.second->Frame(spf, accTime);
		}
	}
	return true;
}
