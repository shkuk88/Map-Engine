...
bool XMapImporter::ImportObject()
{
	...
	for (int iObjType = 0; iObjType < iObjTypeCnt; iObjType++)
	{
		...
		I_Object.AddObject(DxManager::Get().GetDevice(), DxManager::Get().GetDContext(), szObjName.c_str());
		...
		for (int iLoop = 0; iLoop < iObjOBBCnt; iLoop++)
		{
			...
			I_Object.m_OBBViewerList[temp][iLoop].SetRotateCollider(DxManager::Get().GetDContext(), I_Object.m_ObjectCollider[temp].ColliderAABB[iLoop], matRotation);
		}
