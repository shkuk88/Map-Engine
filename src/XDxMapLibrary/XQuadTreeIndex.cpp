...
HRESULT XQuadTreeIndex::CreateLeafNodeIndex(XNode* pNode)
{
	...
	V_RETURN(DxManager::Get().GetDevice()->CreateBuffer(&bd, &bData, pNode->m_IndexBuffer.GetAddressOf()));
};
...
bool XQuadTreeIndex::Render(ID3D11DeviceContext* pContext)
{
	...
	pContext->UpdateSubresource(m_Map->m_pVertexBuffer.Get(), 0, NULL, &m_Map->m_VertexList.at(0), NULL, NULL);
